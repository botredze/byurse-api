...
export class FavoriteProductsController {
  ...
  async addToFavorites(@Param('userId') userId: number, @Param('productId') productId: number): Promise<void> {
    await this.favoriteProductsService.addToFavorites(userId, productId);
  }
